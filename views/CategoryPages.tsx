  ...
  return (
    ...
    >
       <button 
         ...
         className="p-3 hover:bg-stone-50 rounded-xl text-stone-600 hover:text-ink active:scale-75 transition-all cursor-pointer outline-none"
        >
          <ChevronLeft size={20} />
        </button>
       <div className="flex items-center gap-3">
          <Calendar size={16} className="text-organic-600 opacity-80" />
...
const PageWrapper = ({ Component, title, subtitle }: { Component: any, title: string, subtitle: string }) => {
   ...
   return (
     <PageContainer className="max-w-4xl">
        ...
        {isCloudLoading && (
          <div className="flex items-center justify-center gap-3 mb-8 text-organic-700 animate-pulse">
            <Loader2 size={18} className="animate-spin opacity-80" />
            <span className="text-[10px] font-black uppercase tracking-[0.4em] opacity-80">Opening Archive...</span>
          </div>
        )}
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isActive 
                        ...
                        : 'bg-white border-stone-100 text-stone-700 hover:border-organic-500 hover:text-organic-800 hover:-translate-y-0.5'}
                  `}
                >
                  {emo}
                </button>
...
const ReflectionsContent = ({ entry, save }: { entry: DailyEntry, save: any }) => (
    <div className="animate-in fade-in duration-700" onClick={(e) => e.stopPropagation()}>
      <Card className="min-h-[60vh] flex flex-col border-none shadow-soft/50">
        <textarea 
          className="w-full flex-grow bg-transparent border-none p-0 text-xl md:text-2xl font-serif text-ink leading-[1.8] focus:ring-0 resize-none placeholder-stone-400 selection:bg-organic-100" 
        ...
        <div className="pt-8 border-t border-stone-50 mt-8 flex justify-end items-center gap-3">
            <span className="font-mono text-[9px] text-stone-600 font-black uppercase tracking-[0.4em]">Vault Protection Active</span>
        </div>
      </Card>
    </div>
);
